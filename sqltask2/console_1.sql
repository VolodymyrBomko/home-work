...
select max(Sum) as Summ, idClient, FirstName, LastName
FROM client,
     application
...
select count(Client_idClient), FirstName, LastName, idClient
from client,
     application
where idClient = Client_idClient
group by idClient;
...
from application,
     client
where idClient = Client_idClient
  and substr(LastName, 2, 1) in ('a', 'e','o', 'u', 'i', 'y');
#
#
...
select Sum, FirstName, LastName
from client,
     application
where idClient = Client_idClient
  and Sum > (select avg(Sum) from application);
...
select FirstName, LastName, City, count(idApplication) as kredutu
from client,
     application
where idClient = Client_idClient
group by idClient
